...
func setupAdminPersistenceTestDB(t *testing.T) *sqlx.DB {
	...
	for _, statement := range []string{
		...
		`CREATE TABLE leads (id BIGINT PRIMARY KEY AUTO_INCREMENT, type ENUM('equipment','construction','finance_lease','compute') NOT NULL, contact_name VARCHAR(64), contact_phone VARCHAR(20), contact_email VARCHAR(128), company_name VARCHAR(128), description TEXT, amount_range VARCHAR(32), term VARCHAR(32), source VARCHAR(32), created_by BIGINT NULL, status ENUM('new','assigned','following','quoted','closed','cancelled') DEFAULT 'new', assignee_id BIGINT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)`,
	} {
