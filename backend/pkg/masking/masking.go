// Package masking 提供买家侧信息隔离用的脱敏函数。
// 原实现在 internal/compute/masking.go; 设备市场也需要同一口径的企业名脱敏,
// 因此下沉到 pkg 供各业务域共用(compute 保留同名转发函数, 调用方无需改动)。
package masking
...
// MaskCompanyName 买家侧供给方/供应方名称脱敏 (信息隔离):
